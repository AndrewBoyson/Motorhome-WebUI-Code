...
int    FileSaveText(char *dst, char *text)
{
	int len = strlen(text);
	
    // Open output files
    int openFlags = O_CREAT | O_WRONLY | O_TRUNC;
    int outputFd = open(dst, openFlags, 0666);
    if (outputFd == -1)
	{
		LogErrno("ERROR opening copy file destination ");
		return -1;
    ...
    // Transfer data
	int numWritten = write(outputFd, text, len);
	if (numWritten != len)
	{
		LogErrno("ERROR writing file");
		close(outputFd);
...
char  FolderExists(char* folder)
{
	DIR* d = opendir(folder);
	//LogErrno("Checking folder exists");
	if (!d) return 0;
	closedir(d);
	return 1;
}
void* FolderOpen(char* folder) //Returns 0 on failure and a pointer on success
{
	DIR* d = opendir(folder);
	if (!d) LogErrno("Could not open directory");
	return d;
}
void FolderClose(void* d)
{
	if (closedir((DIR*)d)) LogErrno("Could not close current directory");
}
char* FolderNextFile(void* d)
{
	struct dirent *dir;
	while(1)
	{
		dir = readdir((DIR*)d);
		if (!dir) return 0;
		if (dir->d_type == DT_REG) return dir->d_name;
	}
}
char* FolderNextEntry(void* d)
{
	struct dirent *dir;
	while(1)
	{
		dir = readdir((DIR*)d);
		if (!dir) return 0;
		if (dir->d_type == DT_REG || (dir->d_type == DT_DIR && dir->d_name[0] != '.')) return dir->d_name;
	}
}
char* FolderNextFileMatchStem(void* d, char *stem)
{
	int n = strlen(stem);
	while (1)
	{
		char *filename = FolderNextFile(d);
...
int   DirOpen(char *folder)
{
	...
	if (d == NULL)
	{
		LogErrno("Could not open directory");
