...
void LogSetLevel(char logLevel)
{
	_logLevel = logLevel;
	SettingsSetChar("logLevel", logLevel);
}
char LogGetLevel()
{
	return _logLevel;
...
void LogInit()
{
	if (SettingsGetChar("logLevel", &_logLevel)) printf("Had error reading logLevel\n");
}
...
//	Log('e', "%s %s %s", text, strerrorname_np(), strerror(errno)); //need glibc 2.32 - use ldd --version to check existing
	Log('e', "%s %d %s", text, errno, strerror(errno));
}
void LogNumber(char *text, int number) {
	Log('e', "%s %d %s", text, number, strerror(number));
}
