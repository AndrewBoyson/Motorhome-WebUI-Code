...
{
	time_t secondsSinceLastStatus = time(0) - timeOfLastStatus;
	if (secondsSinceLastStatus > 65535) secondsSinceLastStatus = 65535;
	if (secondsSinceLastStatus <     0) secondsSinceLastStatus =     0;
	return (uint16_t)(secondsSinceLastStatus);
}
...
char TrumaGetSendWanted()       { return _sendWanted; }
void TrumaSetSendWanted(char v) { _sendWanted = v; if (v) _sendOngoing = 1;}
